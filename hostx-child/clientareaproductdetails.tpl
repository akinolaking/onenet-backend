...
{if $unpaidInvoice}
    <div class="alert alert-hostx alert-{if $unpaidInvoiceOverdue}danger{else}warning{/if}" id="alert{if $unpaidInvoiceOverdue}Overdue{else}Unpaid{/if}Invoice">
        <div class="float-right">
            <a href="viewinvoice.php?id={$unpaidInvoice}" class="hx-small-btn hx-primary-btn">
                {lang key='payInvoice'}
            </a>
        </div>
...
{/if}
<div class="tab-content margin-bottom hostx-product-detail-page {if $module neq ''}module-active-{$module}{/if}">
    <div class="tab-pane fade show active" role="tabpanel" id="tabOverview">
        {if $tplOverviewTabOutput}
            <div class="overview-tab-data {if $smarty.get.customAction}{$smarty.get.customAction}{/if}">
                        ...
                        <div class="col-md-6">
                            <div class="product-status product-status-{$rawstatus|strtolower} mb-3">
                                ...
                                {if $showcancelbutton || $packagesupgrade}
                                    <div class="button-section-detail-product">
                                        {if $packagesupgrade}
                                ...
                                {else}
                                    {if $domain}
                                        <div class="footer-product-domain">{$domain}</div>
                                    {/if}
                                {/if}
                            </div>
                        </div>
                        <div class="col-md-6 text-center product-info-sec">
                            <div class="info-list-prd">
                                <h6>{lang key='clientareastatus'}</h6>
                                <span class="status-service status-{$rawstatus|strtolower}">{$status}</span>
                            </div>
                            <div class="info-list-prd">
                                <h6>{lang key='clientareahostingregdate'}</h6>
                                <span>{$regdate}</span>
                            </div>
                            {if $firstpaymentamount neq $recurringamount}
                            <div class="info-list-prd">
                                <h6>{lang key='firstpaymentamount'}</h6>
                                <span>{$firstpaymentamount}</span>
                            </div>
                            {/if}
                            {if $billingcycle != "{lang key='orderpaymenttermonetime'}" && $billingcycle != "{lang key='orderfree'}"}
                            <div class="info-list-prd">
                                <h6>{lang key='recurringamount'}</h6>
                                <span>{$recurringamount}</span>
                            </div>
                            {/if}
                            {if $quantitySupported && $quantity > 1}
                            <div class="info-list-prd">
                                <h6>{lang key='quantity'}</h6>
                                <span>{$quantity}</span>
                            </div>
                            {/if}
                            <div class="info-list-prd">
                                <h6>{lang key='orderbillingcycle'}</h6>
                                <span>{$billingcycle}</span>
                            </div>
                            <div class="info-list-prd">
                                <h6>{lang key='clientareahostingnextduedate'}</h6>
                                <span>{$nextduedate}</span>
                            </div>
                            <div class="info-list-prd">
                                <h6>{lang key='orderpaymentmethod'}</h6>
                                <span>{$paymentmethod}</span>
                            </div>
                            {if $suspendreason}
                            <div class="info-list-prd">
                                <h6>{lang key='suspendreason'}</h6>
