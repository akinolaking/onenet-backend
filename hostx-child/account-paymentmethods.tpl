...
<div class="card">
    <div class="card-body">
        ...
        <p>
            {if $allowCreditCard}
                <a href="{routePath('account-paymentmethods-add')}" class="button-style hx-primary-btn mb-4" data-role="add-new-credit-card">
                    {lang key='paymentMethods.addNewCC'}
                </a>
            {/if}
            {if $allowBankDetails}
                <a href="{routePathWithQuery('account-paymentmethods-add', null, 'type=bankacct')}" class="button-style hx-black-btn mb-4">
