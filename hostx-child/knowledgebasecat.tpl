<form role="form" method="post" action="{routePath('knowledgebase-search')}" class="mb-4">
    <div class="input-group input-group-lg kb-search">
        ...
        <div class="input-group-append">
            <button type="submit" id="btnKnowledgebaseSearch" class="btn button-style hx-primary-btn">
                {lang key='search'}
            </button>
        </div>
    </div>
            ...
            {foreach $kbarticles as $kbarticle}
                <a href="{routePath('knowledgebase-article-view', {$kbarticle.id}, {$kbarticle.urlfriendlytitle})}" class="list-group-item kb-article-item" data-id="{$kbarticle.id}">
                    ...
                    {if $kbarticle.editLink}
                        <button class="btn button-style hx-black-btn" id="btnEditArticle-{$kbarticle.id}" data-url="{$kbarticle.editLink}" type="button">
                            {lang key="edit"}
                        </button>
                    {/if}
...
<a href="javascript:history.go(-1)" class="btn button-style hx-black-btn">
    {lang key='clientareabacklink'}
</a>

{if $kbcurrentcat.editLink}
    <a href="{$kbcurrentcat.editLink}" class="btn button-style hx-primary-btn float-right">
