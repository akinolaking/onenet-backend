<div class="domain p-vertical">
    <div class="container">
      <div class="top">
        ...
        <p class="font-weight-bold">{$LANG.domainsecureyourdmn}</p> 
      </div>
      <div class="search_box">
        <form action="cart.php" method="get">  
          ...
          {else}
            <input class="search_input" type="text" placeholder="Search a Domain" name="message">  
          {/if}
      ...
      <div class="domain-companys">
        ...
        {foreach from=$tld_data['show_on_domain_companys'] key=k  item=tld_d}
          ...
          <h3>
            ...
            {else}
              <b>{$tld_d->register|replace:$tld_data['currency_formate']:$currency_formate}</b>/{$LANG.domainyr}
            {/if}
          </h3> 
        </div>
        {/foreach}
